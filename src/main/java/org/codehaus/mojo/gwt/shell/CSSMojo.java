...
import org.codehaus.mojo.gwt.utils.FileRetriever;
...
public class CSSMojo extends AbstractGwtShellMojo {
	...
	public void doExecute() throws MojoExecutionException, MojoFailureException {
		...
		if (cssFiles != null) {
			for (final String file : cssFiles) {
				final String typeName = FilenameUtils.separatorsToSystem(file).substring(0,
						file.lastIndexOf('.')).replace(File.separatorChar, '.');
				...
				for (final Resource resource : (List<Resource>) getProject().getResources()) {
					...
					if (candidate.exists()) {
						...
						try {
							new JavaCommand("com.google.gwt.resources.css.InterfaceGenerator").withinScope(
									Artifact.SCOPE_COMPILE).arg("-standalone").arg("-typeName").arg(typeName).arg(
									"-css").arg(candidate.getAbsolutePath()).withinClasspath(getGwtDevJar()).withinClasspath(
									getGwtUserJar()).execute();
	...
	throws MojoExecutionException {
		...
		if ((cssFiles == null) && (cssFile != null)) {
			...
			cssFiles = FileRetriever.retrieveCssFiles(getProject().getFile(), getLog());
		}

	}


	private void ensureTargetPackageExists(final File generateDirectory, String targetName) {
		targetName = targetName.contains(".") ? targetName.substring(0, targetName.lastIndexOf('.'))
				: targetName;
		final String targetPackage = targetName.replace('.', File.separatorChar);
		getLog().debug(
				"ensureTargetPackageExists, targetName : " + targetName + ", targetPackage : "
						+ targetPackage);
		final File targetPackageDirectory = new File(generateDirectory, targetPackage);
